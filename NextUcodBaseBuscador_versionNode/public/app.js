...
function setSearch() {
  ...
  if (busqueda.prop('checked')) //revisa que el atributo checked sea falso o verdadero
  {
    ...
    return Buscador = {
      ...
      ,Init: function(){
        ...
        $("#buscar").click(()=>{
          ...
          //self.loadInmuebles()
          let busqueda = $('#checkPersonalizada');
          console.log('click en buscar la check esta ' + busqueda.prop('checked')  )
          if (busqueda.prop('checked')) //revisa que el atributo checked sea falso o verdadero
          {
            self.loadCiudades()
            self.loadTipos()
            ...
            console.log('el rango de precio es: '+ $("#rangoPrecio").val())
               ...
               $.each(data, (i,inmueble)=>{
                 var htmlNew = self.$clasificadoTemp.html().replace(":Direccion:",inmueble.Direccion)
                  ...
                  if(filter===undefined){
                    ...
                    var precio0Int = parseInt(precio[0]);// se deben pasar estos valores a int para que quede bien hecho las comparaciones <=
                    var precio1Int = parseInt(precio[1]);
                    var precioInmuebleInt = parseInt(precioInmueble);

                    console.log ('el precio verdadero tipo INT esta en el rango'+ ( precioInmuebleInt >= precio0Int && precioInmuebleInt <= precio1Int));

                    console.log ('precio1 y precio 2 '+ precio0Int+ ' ' + precio1Int + ' ' + precioInmuebleInt )
                    var show = show && (precioInmuebleInt >= precio0Int && precioInmuebleInt <= precio1Int);
